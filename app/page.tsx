...
// 1) Import getServerSession from "next-auth" (no authOptions!)
import { getServerSession } from "next-auth";

export default async function Home() {
  // 2) Call getServerSession() with no arguments
  //    This only works if NextAuth is in the default route (like /api/auth/[...nextauth])
  //    and there's no custom config needed on the client side.
    ...
    <main className="min-h-screen flex flex-col items-center justify-start p-7 space-y-12 bg-black">
      ...
      <div className="w-full max-w-4xl my-12">
        ...
        <div id="attendance" className="w-full">
          {/* Pass the fetched session to <Attendance /> */}
