...
// 1) Use getServerSideProps to fetch session & prevent static generation
...
interface AttendanceProps {
  session: any; // or Session if you've typed next-auth
}

export default function Attendance({ session }: AttendanceProps) {
  const [accommodations, setAccommodations] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);

  // 2) Auto-submit if session is already available
  useEffect(() => {
  ...
  // 3) Handle sign-in and accommodations submission
      ...
      <div className="p-6 space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full text-black px-4 py-3 bg-gray-800 text-gray-200
